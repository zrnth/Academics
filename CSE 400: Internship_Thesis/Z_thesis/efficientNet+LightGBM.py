#!/usr/bin/env python3
"""
Complete EfficientNet + LightGBM Hybrid Bone Marrow Cell Classification Model
Advanced hybrid approach combining deep learning feature extraction with gradient boosting:
- EfficientNet for deep feature extraction from images
- LightGBM for classification on combined features (deep + handcrafted + embeddings)
...
print("Complete EfficientNet + LightGBM Hybrid Model")
...
class EfficientNetFeatureExtractor(nn.Module):
    """
    EfficientNet-based feature extractor optimized for bone marrow images
    """
    def __init__(self, model_name='efficientnet_b4', feature_dim=512, pretrained=True):
        super(EfficientNetFeatureExtractor, self).__init__()
        
        self.model_name = model_name
        self.feature_dim = feature_dim
        
        print(f"Building EfficientNet Feature Extractor ({model_name})")
        
        # Load pretrained EfficientNet
        if model_name == 'efficientnet_b4':
            self.backbone = models.efficientnet_b4(pretrained=pretrained)
            backbone_features = 1792  # EfficientNet-B4 output features
        elif model_name == 'efficientnet_b3':
            self.backbone = models.efficientnet_b3(pretrained=pretrained)
            backbone_features = 1536  # EfficientNet-B3 output features
        elif model_name == 'efficientnet_b2':
            self.backbone = models.efficientnet_b2(pretrained=pretrained)
            backbone_features = 1408  # EfficientNet-B2 output features
        else:
        ...
        self.augmentation = nn.Sequential(
            # Can add more augmentation layers here if needed
        )
        
        print(f"EfficientNet Feature Extractor built: {backbone_features} -> {feature_dim}")
...
class HybridFeatureExtractor:
    ...
    Combines EfficientNet deep features with handcrafted features and embeddings
    """
    def __init__(self, efficientnet_model='efficientnet_b4', deep_feature_dim=512, device='cuda'):
    ...
    def extract_deep_features(self, dataloader, save_path=None):
        """Extract deep features from images using EfficientNet"""
        print("Extracting deep features using EfficientNet...")
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(tqdm(dataloader, desc="Extracting features")):
                try:
                    ...
                    deep_features = self.efficientnet(images)
    ...
    def _create_feature_names(self, deep_dim, handcrafted_dim, embedding_dim):
        ...
        # Deep feature names
        feature_names.extend([f'efficientnet_feature_{i}' for i in range(deep_dim)])
...
class LightGBMClassifier:
    """
    Optimized LightGBM classifier for bone marrow cell classification
    """
    def __init__(self, num_classes=21, class_weights=None):
        ...
        print(f"LightGBM Classifier initialized for {num_classes} classes")
...
class XAIAnalyzer:
    ...
    def analyze_feature_importance(self, save_path=None, top_k=30):
        ...
        plt.title(f'Top {top_k} Most Important Features (LightGBM)', fontsize=14, fontweight='bold')
    ...
    def analyze_region_importance(self, feature_imp_df, save_path=None):
        ...
        for _, row in feature_imp_df.iterrows():
            ...
            if feature_name.startswith('efficientnet_'):
                importance_by_type['efficientnet'] += importance
    ...
    def explain_single_prediction(self, X_sample, sample_idx=0, top_k=15):
        ...
        prediction_proba = self.model.model.predict(X_scaled)[0]
...
def evaluate_model_comprehensive(y_true, y_pred, y_pred_proba, class_names, save_dir=None):
    ...
    if save_dir:
        ...
        plt.title('Confusion Matrix - EfficientNet + LightGBM', fontsize=14, fontweight='bold')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='EfficientNet + LightGBM Hybrid Classification')
    ...
    parser.add_argument('--save_dir', default='efficientnet_lightgbm_results', help='Save directory')
    parser.add_argument('--batch_size', type=int, default=32, help='Batch size for feature extraction')
    parser.add_argument('--efficientnet_model', default='efficientnet_b4', 
    ...
    print("Phase 2: LightGBM Training")
    lgb_classifier = LightGBMClassifier(num_classes=num_classes, class_weights=class_weights)
    
    training_results = lgb_classifier.train(
    ...
    xai_analyzer = XAIAnalyzer(lgb_classifier, train_features['feature_names'], class_names)
    ...
    # Save LightGBM model
    lgb_classifier.model.save_model(os.path.join(args.save_dir, 'lightgbm_model.txt'))
    joblib.dump(lgb_classifier.scaler, os.path.join(args.save_dir, 'feature_scaler.pkl'))
    
    # Save EfficientNet feature extractor
    torch.save(feature_extractor.efficientnet.state_dict(), 
               os.path.join(args.save_dir, 'efficientnet_feature_extractor.pth'))
    ...
    print(f"   EfficientNet: {args.efficientnet_model}")
