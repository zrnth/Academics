#!/usr/bin/env python3
"""
Complete InceptionV3 + SVM Hybrid Bone Marrow Cell Classification Model
Advanced hybrid approach combining deep learning feature extraction with support vector machines:
- InceptionV3 for deep feature extraction from images with multi-scale processing
- SVM for classification on combined features (deep + handcrafted + embeddings)
...
print("Complete InceptionV3 + SVM Hybrid Model")
...
class BoneMarrowDataset(Dataset):
    ...
    def _load_image_cached(self, image_name):
        ...
        dummy_image = torch.zeros(3, 299, 299, dtype=torch.float32)
...
class InceptionV3FeatureExtractor(nn.Module):
    """
    InceptionV3-based feature extractor optimized for bone marrow images
    """
    def __init__(self, feature_dim=512, pretrained=True):
        ...
        print(f"Building InceptionV3 Feature Extractor")
        
        # Load pretrained InceptionV3
        self.backbone = models.inception_v3(pretrained=pretrained, aux_logits=False)
        backbone_features = self.backbone.fc.in_features  # 2048 for InceptionV3
        
        # Remove the original classifier
        self.backbone.fc = nn.Identity()
        
        # Custom feature extraction head with inception-style modules
        self.feature_head = nn.Sequential(
            ...
            # Second reduction with inception-style parallel paths
        ...
        # Inception-style auxiliary feature extractor
        ...
        # Fine-tuning strategy for InceptionV3
        self._setup_fine_tuning()
        
        print(f"InceptionV3 Feature Extractor built: {backbone_features} -> {feature_dim}")
        
    def _setup_fine_tuning(self):
        """Setup fine-tuning strategy for InceptionV3"""
        # Freeze early inception blocks, fine-tune later ones
        for name, param in self.backbone.named_parameters():
            if any(early_block in name for early_block in ['Conv2d_1a', 'Conv2d_2a', 'Conv2d_2b', 'Conv2d_3b']):
                param.requires_grad = False
            elif any(mixed_block in name for mixed_block in ['Mixed_6', 'Mixed_7']):
                param.requires_grad = True
            else:
                param.requires_grad = True
    
    def forward(self, x):
        """Forward pass through InceptionV3 with multi-scale features"""
        # Extract features using InceptionV3 backbone
        ...
        # Main feature path
        main_features = self.feature_head(backbone_features)
        
        # Auxiliary feature path (inspired by Inception architecture)
        aux_features = self.auxiliary_head(backbone_features)
...
class HybridFeatureExtractor:
    ...
    Combines InceptionV3 deep features with handcrafted features and embeddings
    """
    def __init__(self, deep_feature_dim=512, device='cuda'):
        ...
        self.inceptionv3 = InceptionV3FeatureExtractor(
    ...
    def extract_deep_features(self, dataloader, save_path=None):
        """Extract deep features from images using InceptionV3"""
        print("Extracting deep features using InceptionV3...")
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(tqdm(dataloader, desc="Extracting features")):
                try:
                    ...
                    deep_features = self.inceptionv3(images)
    ...
    def _create_feature_names(self, deep_dim, handcrafted_dim, embedding_dim):
        ...
        # Deep feature names
        feature_names.extend([f'inceptionv3_feature_{i}' for i in range(deep_dim)])
...
class SVMClassifierWrapper:
    """
    Optimized SVM classifier for bone marrow cell classification
    """
    def __init__(self, num_classes=21, class_weights=None):
        ...
        print(f"SVM Classifier initialized for {num_classes} classes")
        
    def train(self, X_train, X_test, y_train, y_test, feature_names=None, 
              use_early_stopping=False, cv_folds=5):
        """Train SVM with cross-validation and hyperparameter optimization"""
        print("Training SVM Classifier...")
        
        # Feature scaling (crucial for SVM)
        print("Scaling features...")
        self.scaler = StandardScaler()  # SVM requires standardized features
        ...
        if cv_folds > 1:
            ...
            for fold, (train_idx, val_idx) in enumerate(skf.split(X_train_scaled, y_train)):
                ...
                print(f"     Support vectors: {fold_model.n_support_.sum()}")
        ...
        print(f"SVM trained with {self.model.n_support_.sum()} support vectors")
        print(f"Support vectors per class: {dict(zip(range(len(self.model.n_support_)), self.model.n_support_))}")
    ...
    def get_feature_importance(self, X_test, y_test, importance_type='permutation'):
        """Get feature importance using permutation importance (SVM doesn't have native importance)"""
        ...
        if importance_type == 'permutation':
            ...
            # For linear SVM, we could use coefficient magnitudes
            if self.model.kernel == 'linear':
                # Average absolute coefficients across classes
                coef_abs = np.abs(self.model.coef_)
                return np.mean(coef_abs, axis=0)
            else:
                # For non-linear kernels, fall back to permutation importance
                return self.get_feature_importance(X_test, y_test, 'permutation')
    ...
    def analyze_support_vectors(self):
        ...
        support_info = {
            'total_support_vectors': self.model.n_support_.sum(),
            'support_vectors_per_class': dict(zip(range(len(self.model.n_support_)), self.model.n_support_)),
...
class XAIAnalyzer:
    """
    Comprehensive XAI analysis for the hybrid model (adapted for SVM)
    """
    def __init__(self, model, feature_names, class_names):
        ...
        print("XAI Analyzer initialized for SVM")
        
    def setup_shap_explainer(self, X_background, max_background_samples=100):
        """Setup SHAP explainer with background data (using KernelExplainer for SVM)"""
        print("Setting up SHAP explainer for SVM...")
        ...
        # Create KernelExplainer for SVM (model-agnostic)
        def predict_fn(X):
            return self.model.model.predict_proba(X)
        
        self.shap_explainer = shap.KernelExplainer(predict_fn, X_bg_scaled)
    ...
    def analyze_feature_importance(self, X_test, y_test, save_path=None, top_k=30):
        """Analyze and visualize feature importance using permutation importance"""
        print("Analyzing feature importance using permutation importance...")
        
        # Get feature importance
        importance = self.model.get_feature_importance(X_test, y_test)
        ...
        plt.title(f'Top {top_k} Most Important Features (SVM - Permutation)', fontsize=14, fontweight='bold')
    ...
    def analyze_region_importance(self, feature_imp_df, save_path=None):
        ...
        for _, row in feature_imp_df.iterrows():
            ...
            if feature_name.startswith('inceptionv3_'):
                importance_by_type['inceptionv3'] += importance
    ...
    def generate_shap_explanations(self, X_samples, sample_indices=None, max_samples=20):
        """Generate SHAP explanations for samples (reduced max_samples for KernelExplainer efficiency)"""
        if self.shap_explainer is None:
            print("ERROR: SHAP explainer not setup. Call setup_shap_explainer() first.")
            return None
        
        print("Generating SHAP explanations (this may take a while for SVM)...")
        
        # Limit samples for efficiency (KernelExplainer is computationally expensive)
        ...
        shap_values = self.shap_explainer.shap_values(X_explain_scaled, nsamples=100)
    ...
    def analyze_decision_boundary(self, save_path=None):
        """Analyze SVM decision boundary properties"""
        if self.model.model is None:
            print("ERROR: Model not trained.")
            return None
        
        print("Analyzing SVM decision boundary...")
        
        # Get support vector analysis
        sv_info = self.model.analyze_support_vectors()
        
        # Plot support vectors distribution
        plt.figure(figsize=(10, 6))
        classes = list(sv_info['support_vectors_per_class'].keys())
        sv_counts = list(sv_info['support_vectors_per_class'].values())
        
        bars = plt.bar(classes, sv_counts, alpha=0.7, color='skyblue')
        ...
        for bar, count in zip(bars, sv_counts):
            plt.text(bar.get_x() + bar.get_width()/2, bar.get_height() + max(sv_counts)*0.01, 
...
def evaluate_model_comprehensive(y_true, y_pred, y_pred_proba, class_names, save_dir=None):
    ...
    if save_dir:
        ...
        plt.title('Confusion Matrix - InceptionV3 + SVM', fontsize=14, fontweight='bold')
...
def create_data_loaders(csv_file, image_dir, embeddings_dir, batch_size=32, test_size=0.2, mode='hybrid'):
    ...
    # Transforms for images (299x299 for InceptionV3)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='InceptionV3 + SVM Hybrid Classification')
    ...
    parser.add_argument('--save_dir', default='inceptionv3_svm_results', help='Save directory')
    ...
    parser.add_argument('--svm_c', type=float, default=10.0, help='SVM regularization parameter')
    parser.add_argument('--svm_kernel', default='rbf', choices=['rbf', 'linear', 'poly', 'sigmoid'],
    ...
    print("Phase 2: SVM Training")
    svm_classifier = SVMClassifierWrapper(num_classes=num_classes, class_weights=class_weights)
    
    # Update SVM parameters if specified
    svm_classifier.params['C'] = args.svm_c
    svm_classifier.params['kernel'] = args.svm_kernel
    
    training_results = svm_classifier.train(
    ...
    xai_analyzer = XAIAnalyzer(svm_classifier, train_features['feature_names'], class_names)
    ...
    sv_analysis = xai_analyzer.analyze_decision_boundary(
        save_path=os.path.join(args.save_dir, 'support_vectors.png')
    )
    
    # SHAP analysis (reduced samples for efficiency)
    xai_analyzer.setup_shap_explainer(train_features['features'])
    shap_values, X_explain = xai_analyzer.generate_shap_explanations(
        test_features['features'], max_samples=20  # Reduced for SVM efficiency
    )
    ...
    # Save SVM model
    joblib.dump(svm_classifier.model, os.path.join(args.save_dir, 'svm_model.pkl'))
    joblib.dump(svm_classifier.scaler, os.path.join(args.save_dir, 'feature_scaler.pkl'))
    
    # Save InceptionV3 feature extractor
    torch.save(feature_extractor.inceptionv3.state_dict(), 
               os.path.join(args.save_dir, 'inceptionv3_feature_extractor.pth'))
    ...
    final_results = {
        ...
        'training_results': {
            ...
            'n_support_vectors': training_results['n_support_vectors']
        },
    ...
    print(f"   Support Vectors: {training_results['n_support_vectors']}")
    print(f"Total training time: {total_time/60:.2f} minutes")
    print(f"Results saved to: {args.save_dir}")
    print(f"Model features:")
    print(f"   InceptionV3: Multi-scale feature extraction with auxiliary paths")
    print(f"   SVM: {args.svm_kernel} kernel with C={args.svm_c}")
