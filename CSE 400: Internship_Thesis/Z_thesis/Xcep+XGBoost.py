#!/usr/bin/env python3
"""
Complete Xception + XGBoost Hybrid Bone Marrow Cell Classification Model
Advanced hybrid approach combining deep learning feature extraction with gradient boosting:
- Xception for deep feature extraction from images with depthwise separable convolutions
- XGBoost for classification on combined features (deep + handcrafted + embeddings)
...
print("Complete Xception + XGBoost Hybrid Model")
        ...
        if image_path:
            try:
                ...
                if image is not None:
                    ...
                    # Resize to 299x299 for Xception (standard input size)
...
class XceptionFeatureExtractor(nn.Module):
    """
    Xception-based feature extractor with depthwise separable convolutions
    Optimized for bone marrow images
    """
    def __init__(self, feature_dim=512, pretrained=True):
        ...
        print(f"Building Xception Feature Extractor")
        
        # Build custom Xception architecture since torchvision doesn't have it
        # We'll use depthwise separable convolutions as in the original paper
        ...
        self.block1 = self._make_xception_block(64, 128, 2)
        self.block2 = self._make_xception_block(128, 256, 2)
        self.block3 = self._make_xception_block(256, 728, 2)
        
        # Middle flow (8 blocks)
        self.middle_blocks = nn.ModuleList([
            self._make_xception_block(728, 728, 1) for _ in range(8)
        ])
        
        # Exit flow
        self.exit_block = self._make_xception_block(728, 1024, 2)
        
        # Separable convolutions for exit flow
        self.exit_sep_conv1 = self._make_separable_conv(1024, 1536, 3, 1, 1)
        self.exit_sep_conv2 = self._make_separable_conv(1536, 2048, 3, 1, 1)
        ...
        print(f"Xception Feature Extractor built: 2048 -> {feature_dim}")
        
        # Initialize weights
        self._initialize_weights()
        
    def _make_separable_conv(self, in_channels, out_channels, kernel_size, stride, padding):
    ...
    def _make_xception_block(self, in_channels, out_channels, stride):
        """Create Xception block with residual connection"""
        block = nn.Sequential(
            # First separable conv
            self._make_separable_conv(in_channels, out_channels, 3, 1, 1),
            # Second separable conv
            self._make_separable_conv(out_channels, out_channels, 3, stride, 1),
        )
    ...
    def forward(self, x):
        ...
        deep_features = self.feature_head(x)
...
class HybridFeatureExtractor:
    ...
    Combines Xception deep features with handcrafted features and embeddings
    """
    def __init__(self, deep_feature_dim=512, device='cuda'):
        ...
        # Initialize Xception
        self.xception = XceptionFeatureExtractor(
    ...
    def extract_deep_features(self, dataloader, save_path=None):
        """Extract deep features from images using Xception"""
        print("Extracting deep features using Xception...")
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(tqdm(dataloader, desc="Extracting features")):
                try:
                    ...
                    deep_features = self.xception(images)
    ...
    def _create_feature_names(self, deep_dim, handcrafted_dim, embedding_dim):
        ...
        # Deep feature names
        feature_names.extend([f'xception_feature_{i}' for i in range(deep_dim)])
...
class XGBoostClassifier:
    """
    Optimized XGBoost classifier for bone marrow cell classification
    """
    def __init__(self, num_classes=21, class_weights=None):
        ...
        print(f"XGBoost Classifier initialized for {num_classes} classes")
        
    def train(self, X_train, X_test, y_train, y_test, feature_names=None, 
              use_early_stopping=True, cv_folds=5):
        ...
        if cv_folds > 1:
            ...
            for fold, (train_idx, val_idx) in enumerate(skf.split(X_train_scaled, y_train)):
                ...
                # Validate
                val_pred = fold_model.predict(val_data)
                val_pred_class = np.argmax(val_pred, axis=1)
                fold_score = accuracy_score(y_fold_val, val_pred_class)
        ...
        train_data = xgb.DMatrix(X_train_scaled, label=y_train, feature_names=feature_names)
        test_data = xgb.DMatrix(X_test_scaled, label=y_test, feature_names=feature_names)
    ...
    def predict(self, X):
        ...
        test_data = xgb.DMatrix(X_scaled)
        ...
        if hasattr(self, 'feature_names') and self.feature_names:
            ...
            for i, name in enumerate(self.feature_names):
                if name in importance_dict:
                    importance[i] = importance_dict[name]
        else:
...
class XAIAnalyzer:
    ...
    def analyze_feature_importance(self, save_path=None, top_k=30):
        ...
        importance = self.model.get_feature_importance('gain')
    ...
    def analyze_region_importance(self, feature_imp_df, save_path=None):
        ...
        for _, row in feature_imp_df.iterrows():
            ...
            if feature_name.startswith('xception_'):
                importance_by_type['xception'] += importance
    ...
    def generate_shap_explanations(self, X_samples, sample_indices=None, max_samples=50):
        ...
        dmatrix = xgb.DMatrix(X_explain_scaled)
    ...
    def explain_single_prediction(self, X_sample, sample_idx=0, top_k=15):
        ...
        test_data = xgb.DMatrix(X_scaled)
        prediction_proba = self.model.model.predict(test_data)[0]
        ...
        print(f"Predicted Class: {self.class_names[predicted_class]} (confidence: {prediction_proba[predicted_class]:.3f})")
...
def evaluate_model_comprehensive(y_true, y_pred, y_pred_proba, class_names, save_dir=None):
    ...
    if save_dir:
        ...
        plt.title('Confusion Matrix - Xception + XGBoost', fontsize=14, fontweight='bold')
...
def create_data_loaders(csv_file, image_dir, embeddings_dir, batch_size=32, test_size=0.2, mode='hybrid'):
    ...
    # Transforms for images (299x299 for Xception)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Xception + XGBoost Hybrid Classification')
    ...
    parser.add_argument('--save_dir', default='xception_xgboost_results', help='Save directory')
    ...
    xgb_classifier = XGBoostClassifier(num_classes=num_classes, class_weights=class_weights)
    
    training_results = xgb_classifier.train(
    ...
    xai_analyzer = XAIAnalyzer(xgb_classifier, train_features['feature_names'], class_names)
    ...
    # Save XGBoost model
    xgb_classifier.model.save_model(os.path.join(args.save_dir, 'xgboost_model.json'))
    joblib.dump(xgb_classifier.scaler, os.path.join(args.save_dir, 'feature_scaler.pkl'))
    
    # Save Xception feature extractor
    torch.save(feature_extractor.xception.state_dict(), 
               os.path.join(args.save_dir, 'xception_feature_extractor.pth'))
    ...
    print(f"   Xception: Custom implementation with depthwise separable convolutions")
